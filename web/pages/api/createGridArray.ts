  ...
  for (let i = 0; i < boxes.length; i++) {
    ...
    for (let c = 0; c < rayHits.length; c++) {
      ...
      if (rayHit.boxes.length == 0) {
        ...
        if (ray.directionString == "down" || ray.directionString == "right") {
          bottomRight = new Point(topLeft.x + avgWidth, topLeft.y + avgHeight);
        } else {
          topLeft = new Point(
  ...
  for (let r = 0; r < grid.length; r++) {
    for (let c = 0; c < grid[r].length; c++) {
      ...
      if (!box) {
        ...
        print += box.klass + " ";
        box.draw(gridImage.getContext("2d") as any, "green", box.klass); // had to cast as any here, this might break things in the future
      }
    }
    print += "\n";
  }
