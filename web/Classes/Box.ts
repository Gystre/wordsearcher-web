...
export class Box {
  ...
  draw = (
    ...
    // draw the text, don't check local storage if we're on the server
    ...
    ctx.fillText(label, this.topLeft.x - 1, this.topLeft.y - (textHeight + 2));
  };
  ...
  drawLine = (
    ...
    // da text
    const lightMode = localStorage.getItem("chakra-ui-color-mode") === "light";
    ctx.fillStyle = lightMode ? "#000000" : "#FFFFFF";
    ctx.fillText(label, this.topLeft.x - 1, this.topLeft.y - (textHeight + 2));
  };
